...
class MiscSettings(Configurable):
    ...
    def get_options(self):
        ...
        opt = opt_factory('interface', cf.cf.get(
            'interface'), desc, 'string', tabid='Network settings')
        ol.add(opt)

        desc = 'Local IP address to use when doing reverse connections'
        opt = opt_factory(
            'local_ip_address', cf.cf.get('local_ip_address'), desc, 'string',
        ...
        opt = opt_factory(
            'demo', cf.cf.get('demo'), desc, 'boolean', tabid='Misc settings')
        ol.add(opt)
        ...
        opt = opt_factory(
            'non_targets', cf.cf.get('non_targets'), desc, 'list', help=help,
            tabid='Misc settings')
        ol.add(opt)

        ######### Metasploit ###########
        desc = 'Full path of Metasploit framework binary directory (%s in most linux installs)' % cf.cf.get('msf_location')
        opt = opt_factory('msf_location', cf.cf.get(
            'msf_location'), desc, 'string', tabid='Metasploit')
        ol.add(opt)
