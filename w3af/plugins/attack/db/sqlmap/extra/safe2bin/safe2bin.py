...
# Raw chars that will be safe encoded to their slash (\) representations
# (e.g. newline to \n)
...
SAFE_CHARS = "".join(
    filter(
        lambda _: _ not in SAFE_ENCODE_SLASH_REPLACEMENTS,
...
# Strings used for temporary marking of hex encoded prefixes (to prevent
# double encoding)
    ...
    if isinstance(value, basestring):
        if any([_ not in SAFE_CHARS for _ in value]):
            ...
            retVal = reduce(
                lambda x, y: x + (
                    y if (
                        y in string.printable or isinstance(
                            value, unicode) and ord(y) >= 160) else '\\x%02x' %
                    ord(y)), retVal, (unicode if isinstance(
                        value, unicode) else str)())
...
def safechardecode(value, binary=False):
    ...
    if isinstance(value, basestring):
        ...
        while True:
            ...
            if match:
                retVal = retVal.replace(match.group("result"), (unichr if isinstance(
                    value, unicode) else chr)(ord(binascii.unhexlify(match.group("result").lstrip("\\x")))))
            else:
                break
