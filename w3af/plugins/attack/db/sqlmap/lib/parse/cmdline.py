...
def cmdLineParser(argv=None):
    ...
    # Reference: https://stackoverflow.com/a/4012683 (Note: previously used
    ...
    try:
        ...
        parser.formatter.format_option_strings = type(
            parser.formatter.format_option_strings)(
            _, parser, type(parser))
        ...
        for verbosity in (_ for _ in argv if re.search(r"\A\-v+\Z", _)):
            try:
                if argv.index(verbosity) == len(
                        argv) - 1 or not argv[argv.index(verbosity) + 1].isdigit():
