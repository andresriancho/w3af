...
VERSION = "1.1.11.39"
TYPE = "dev" if VERSION.count('.') > 2 and VERSION.split(
    '.')[-1] != '0' else "stable"
TYPE_COLORS = {"dev": 33, "stable": 90, "pip": 34}
VERSION_STRING = "sqlmap/%s#%s" % ('.'.join(VERSION.split('.')[:-1]) if VERSION.count(
    '.') > 2 and VERSION.split('.')[-1] == '0' else VERSION, TYPE)
...
# Markers for special cases when parameter values contain html encoded
# characters
...
# Regular expression used for extraction of table names (useful for (e.g.)
# MsAccess)
...
# Regular expression used for recognition of generic maximum connection
# messages
MAX_CONNECTIONS_REGEX = r"max.+connections"

# Maximum consecutive connection errors before asking the user if he wants
# to continue
MAX_CONSECUTIVE_CONNECTION_ERRORS = 15

# Timeout before the pre-connection candidate is being disposed (because
# of high probability that the web server will reset it)
...
# Regular expression used for recognition of generic "your ip has been
# blocked" messages
...
# Minimum response time that can be even considered as delayed (not a
# complete requirement)
MIN_VALID_DELAYED_RESPONSE = 0.5

# Standard deviation after which a warning message should be displayed
# about connection lags
WARN_TIME_STDEV = 0.5

# Minimum length of usable union injected response (quick defense against
# substr fields)
...
# In case of missing piece of partial union dump, buffered array must be
# flushed after certain size
MAX_BUFFERED_PARTIAL_UNION_LENGTH = 1024

# Suffix used for naming meta databases in DBMS(es) without explicit
# database name
METADB_SUFFIX = "_masterdb"

# Number of times to retry the pushValue during the exceptions (e.g.
# KeyboardInterrupt)
PUSH_VALUE_EXCEPTION_RETRY_COUNT = 3

# Minimum time response set needed for time-comparison based on standard
# deviation
MIN_TIME_RESPONSES = 30

# Minimum comparison ratio set needed for searching valid union column
# number based on standard deviation
MIN_UNION_RESPONSES = 5

# After these number of blanks at the end inference should stop (just in case)
INFERENCE_BLANK_BREAK = 10

# Use this replacement character for cases when inference is not able to
# retrieve the proper character value
...
ORACLE_SYSTEM_DBS = (
    ...
    "WMSYS",
    "XDB",
    "XS$NULL")  # Reference: https://blog.vishalgupta.com/2011/06/19/predefined-oracle-system-schemas/
...
DBMS_DIRECTORY_DICT = dict(
    (getattr(
        DBMS, _), getattr(
            DBMS_DIRECTORY_NAME, _)) for _ in dir(DBMS) if not _.startswith("_"))

SUPPORTED_DBMS = MSSQL_ALIASES + MYSQL_ALIASES + PGSQL_ALIASES + ORACLE_ALIASES + SQLITE_ALIASES + ACCESS_ALIASES + \
    FIREBIRD_ALIASES + MAXDB_ALIASES + SYBASE_ALIASES + DB2_ALIASES + HSQLDB_ALIASES + INFORMIX_ALIASES
...
FILE_PATH_REGEXES = (
    r"<b>(?P<result>[^<>]+?)</b> on line \d+",
    r"(?P<result>[^<>'\"]+?)['\"]? on line \d+",
    r"(?:[>(\[\s])(?P<result>[A-Za-z]:[\\/][\w. \\/-]*)",
    r"(?:[>(\[\s])(?P<result>/\w[/\w.~-]+)",
    r"href=['\"]file://(?P<result>/[^'\"]+)")
...
# Reference:
# http://www.cs.ru.nl/bachelorscripties/2010/Martin_Devillers___0437999___Analyzing_password_strength.pdf
...
# (along with anything carrying the <hostname> inside)
SENSITIVE_OPTIONS = (
    "hostname",
    "data",
    "dnsDomain",
...
# Minimum length of sentence for automatic choosing of --string (in case
# of high matching ratio)
...
# Do not escape the injected statement if it contains any of the following
# SQL keywords
EXCLUDE_UNESCAPE = (
    "WAITFOR DELAY ",
    " INTO DUMPFILE ",
...
# Maximum number of alpha-numerical parts in reflected regex (for speed
# purposes)
REFLECTED_MAX_REGEX_PARTS = 10

# Chars which can be used as a failsafe values in case of too long URL
# encoding value
...
# Simple regular expression used to distinguish scalar from multiple-row
# commands (not sole condition)
...
# After this number of misses reflective removal mechanism is turned off
# (for speed up reasons)
...
# Chars used to quickly distinguish if the user provided tainted parameter
# values
...
# Patterns often seen in HTTP headers containing custom injection marking
# character '*'
...
# Generic address for checking the Internet connection while using switch
# --check-internet
...
# Approximate chunk length (in bytes) used by BigArray objects (only last
# chunk and cached one are held in memory)
...
# Step used in ORDER BY technique used for finding the right number of
# columns in UNION query injections
ORDER_BY_STEP = 10

# Maximum number of times for revalidation of a character in inference (as
# required)
MAX_REVALIDATION_STEPS = 5

# Characters that can be used to split parameter values in provided
# command line (e.g. in --tamper)
PARAMETER_SPLITTING_REGEX = r"[,|;]"

# Regular expression describing possible union char value (e.g. used in
# --union-char)
UNION_CHAR_REGEX = r"\A\w+\Z"

# Attribute used for storing original parameter value in special cases
# (e.g. POST)
UNENCODED_ORIGINAL_VALUE = "original"

# Common column names containing usernames (used for hash cracking in some
# cases)
COMMON_USER_COLUMNS = (
    "login",
    "user",
...
# Unix timestamp used for forcing cookie expiration when provided with
# --load-cookies
FORCE_COOKIE_EXPIRATION_TIME = "9999999999"

# Github OAuth token used for creating an automatic Issue for unhandled
# exceptions
GITHUB_REPORT_OAUTH_TOKEN = "NTMyNWNkMmZkMzRlMDZmY2JkMmY0MGI4NWI0MzVlM2Q5YmFjYWNhYQ=="

# Skip unforced HashDB flush requests below the threshold number of cached
# items
...
# Unique milestone value used for forced deprecation of old HashDB values
# (e.g. when changing hash/pickle mechanism)
# python -c 'import random, string; print
# "".join(random.sample(string.ascii_letters, 10))'
HASHDB_MILESTONE_VALUE = "dPHoJRQYvs"

# Warn user of possible delay due to large page dump in full UNION query
# injections
LARGE_OUTPUT_THRESHOLD = 1024 ** 2

# On huge tables there is a considerable slowdown if every row retrieval
# requires ORDER BY (most noticable in table dumping using ERROR
# injections)
SLOW_ORDER_COUNT_THRESHOLD = 10000

# Give up on hash recognition if nothing was found in first given number
# of rows
HASH_RECOGNITION_QUIT_THRESHOLD = 10000

# Maximum number of redirections to any single URL - this is needed
# because of the state that cookies introduce
...
# Alphabet used for prefix and suffix strings of name resolution requests
# in DNS technique (excluding hexadecimal chars for not mixing with inner
# content)
...
BANNER = re.sub(
    r"\[.\]",
    lambda _: "[\033[01;41m%s\033[01;49m]" %
    random.sample(
        HEURISTIC_CHECK_ALPHABET,
        1)[0],
    BANNER)

# String used for dummy non-SQLi (e.g. XSS) heuristic checks of a tested
# parameter value
...
# Connection chunk size (processing large responses in chunks to avoid
# MemoryError crashes - e.g. large table dump in full UNION injections)
MAX_CONNECTION_CHUNK_SIZE = 10 * 1024 * 1024

# Maximum response total page size (trimmed if larger)
MAX_CONNECTION_TOTAL_SIZE = 50 * 1024 * 1024

# For preventing MemoryError exceptions (caused when using large sequences
...
BOLD_PATTERNS = (
    ...
    "CAPTCHA",
    "specific response",
    "NULL connection is supported")

# Generic www root directory names
GENERIC_DOC_ROOT_DIRECTORY_NAMES = (
    "htdocs", "httpdocs", "public", "wwwroot", "www")
...
# Number of rows to generate inside the full union test for limited output
# (mustn't be too large to prevent payload length problems)
...
# Length used while checking for existence of Suhosin-patch (like)
# protection mechanism
SUHOSIN_MAX_VALUE_LENGTH = 512

# Minimum size of an (binary) entry before it can be considered for
# dumping to disk
...
# Infixes used for automatic recognition of parameters carrying anti-CSRF
# tokens
...
BRUTE_DOC_ROOT_PREFIXES = {
    ...
    OS.WINDOWS: (
        ...
        "/Program Files/Apache Group/Apache2.4",
        "/Inetpub/wwwroot",
        "/Inetpub/wwwroot/%TARGET%",
...
# String used for marking target name inside used brute force web server
# document root
