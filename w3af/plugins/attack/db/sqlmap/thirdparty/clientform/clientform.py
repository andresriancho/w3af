...
string.
    ...
    if hasattr(query, "items"):
        ...
        try:
            ...
            if len(query) and not isinstance(query[0], types.TupleType):
                raise TypeError()
...
def choose_boundary():
    ...
    nonce = "".join([str(random.randint(0, sys.maxsize - 1))
                     for i in (0, 1, 2)])
    return "-" * 27 + nonce
...
def _ParseFileEx(file, base_uri,
    ...
    for (name, action, method, enctype), attrs, controls in fp.forms:
        if action is None:
            ...
            action = unicode(
                action, "utf8") if action and not isinstance(
                action, unicode) else action
...
# Form-filling methods applying only to ListControls.
    ...
    def toggle_single(self, name=None, type=None, kind=None, id=None,
        ...
        """  # by_label ignored and deprecated
        self._find_list_control(
            name, type, kind, id, label, nr).toggle_single()
    ...
    def _find_control(self, name, type, kind, id, label, predicate, nr):
        ...
        if ambiguous:
            raise AmbiguityError(
                "more than one control matching " + description)
        elif not found:
