...
class apache_config_directory(Payload):
    ...
    def fname_generator(self):
        def parse_apache2_init(apache_file_read):
            directory = re.search(
                '(?<=APACHE_PID_FILE needs to be defined in )(.*?)envvars',
                apache_file_read)
            if directory:
        ...
        try:
            parsed_1 = parse_apache2_init(
                self.shell.read('/etc/init.d/apache2'))
    ...
    def api_read(self):
        ...
        for file_path, content in self.read_multi(
                fname_iter, error_handler=read_error_handler):
