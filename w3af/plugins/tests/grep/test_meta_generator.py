...
class TestMetaGenerator(PluginTest):

    meta_generator_url = get_moth_http('/grep/meta_generator/')

    _run_configs = {
        'cfg1': {
            'target': meta_generator_url,
            'plugins': {
                'grep': (PluginConfig('meta_generator'),),
                'crawl': (
    ...
    def test_found_vuln(self):
        ...
        vulns = self.kb.get('meta_generator', 'meta_generator')
...
class TestMetaGeneratorRaw(unittest.TestCase):
    def setUp(self):
        create_temp_dir()
        kb.kb.cleanup()
    ...
    def test_meta_user(self, *args):
        body = '<meta name="generator" content="WordPress 4.8.1" />'
        ...
        infos = kb.kb.get('meta_generator', 'meta_generator')
        self.assertEquals(len(infos), 1)

        info = infos[0]
        self.assertEqual(info.get_name(), u'Found generator: "WordPress 4.8.1". This information was found in the request with id 1.')
    ...
    def test_group_info_set(self, *args):
        ...
        expected_desc = u'Found generator: "WordPress 4.8.1". This information was found in the request with id 1.'

        # pylint: disable=E1103
        info_set = kb.kb.get_one('meta_generator', 'meta_generator')
