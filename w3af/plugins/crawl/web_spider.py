...
class web_spider(CrawlPlugin):
    ...
    def crawl(self, fuzzable_request, debugging_id):
        ...
        self._extract_links_and_verify(resp, fuzzable_request, debugging_id)
    ...
    def _extract_links_and_verify(self, resp, fuzzable_req, debugging_id):
        """
    ...
    def _handle_http_traffic_from_chrome_queue(self, queue_data):
        ...
        #
        # This extra check with response.force_parsing is required because dom_dump
        # will send an HTTP response to the queue which has the same URI as the
        # page that was loaded. Without the `force_parsing` attribute the
        # `_should_verify_extracted_url` method will recognize the response as
        # a duplicate and ignore it
        #
        if not response.force_parsing:
            if not self._should_verify_extracted_url(response.get_uri()):
                return False

        self._extract_html_forms(response, request)
        self._extract_links_and_verify(response, request, debugging_id)
        return True
