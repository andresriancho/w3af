...
class websockets_links(GrepPlugin):
    ...
    def grep(self, request, response):
        ...
        if ws == []:
            msg =  "The url %s has singns of websockets usage, " \
                   "but coudn't find any useful links.\r\n" \
                   "Perhaps links are dynamicaly created by javascript.\r\n" \
                   "Manual inspection of the page is requred!"
        ...
        i = Info('websockets_links', desc, response.id,
                 self.get_name())
        i.set_url(url)
        i['ws_links'] = ws
