...
class os_commanding(AuditPlugin):
    ...
    def __init__(self):
        ...
        # Scan options
        self._append_time_delay = False
        self._append_echo = False
    ...
    def get_options(self):
        ...
        desc = 'Append time-based payloads to original value'
        _help = 'If set to True, w3af will append time-based payloads to original' \
                ' parameter value. Could significantly increase the overall' \
                ' scan time.'
        opt = opt_factory('append_time_delay', self._append_time_delay,
                          desc, 'boolean', help=_help)
        opt_list.add(opt)

        desc = 'Append echo-based payloads to original value'
        _help = 'If set to True, w3af will append echo-based payloads to original' \
                ' parameter value. Could significantly increase the overall' \
                ' scan time.'
        opt = opt_factory('append_echo', self._append_echo, desc,
                          'boolean', help=_help)
        opt_list.add(opt)
    ...
    def set_options(self, options_list):
        ...
        """
        self._append_time_delay = options_list['append_time_delay'].get_value()
        self._append_echo = options_list['append_echo'].get_value()
