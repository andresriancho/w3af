...
def find_vulns(response):
    ...
    policies_all = merge_policies_dict(
        non_report_only_policies, report_only_policies)
    ...
    if CSP_DIRECTIVE_DEFAULT in policies_all:
        ...
        if "*" in directive_values:
            csp_vuln = CSPVulnerability(
                "Directive 'default-src' allows all sources.", severity.HIGH)
            vulns_lst = [csp_vuln]
    ...
    if CSP_DIRECTIVE_STYLE in policies_all:
        ...
        if "*" in directive_values:
            csp_vuln = CSPVulnerability(
                "Directive 'style-src' allows all CSS sources.", severity.LOW)
            vulns_lst = [csp_vuln]
    ...
    if CSP_DIRECTIVE_IMAGE in policies_all:
        ...
        if "*" in directive_values:
            csp_vuln = CSPVulnerability(
                "Directive 'img-src' allows all image sources.", severity.LOW)
            vulns_lst = [csp_vuln]
    ...
    if CSP_DIRECTIVE_MEDIA in policies_all:
        ...
        if "*" in directive_values:
            csp_vuln = CSPVulnerability(
                "Directive 'media-src' allows all audio/video sources.",
                severity.HIGH)
            vulns_lst = [csp_vuln]
    ...
    if CSP_DIRECTIVE_FRAME in policies_all:
        ...
        if "*" in directive_values:
            csp_vuln = CSPVulnerability(
                "Directive 'frame-src' allows all sources.", severity.HIGH)
            vulns_lst.append(csp_vuln)
    ...
    if CSP_DIRECTIVE_FONT in policies_all:
        ...
        if "*" in directive_values:
            csp_vuln = CSPVulnerability(
                "Directive 'font-src' allows all font sources.",
                severity.MEDIUM)
            vulns_lst = [csp_vuln]
    ...
    if CSP_DIRECTIVE_CONNECTION in policies_all:
        ...
        if "*" in directive_values:
            csp_vuln = CSPVulnerability(
                "Directive 'connect-src' allows all connection sources.",
                severity.HIGH)
            vulns_lst = [csp_vuln]
    ...
    if CSP_DIRECTIVE_FORM in policies_all:
        ...
        if "*" in directive_values:
            csp_vuln = CSPVulnerability(
                "Directive 'form-action' allows all action target.",
                severity.HIGH)
            vulns_lst = [csp_vuln]
    ...
    if CSP_DIRECTIVE_SANDBOX in policies_all:
        ...
        if "allow-*" in directive_values:
            csp_vuln = CSPVulnerability(
                "Directive 'sandbox' apply no restrictions.", severity.HIGH)
            vulns_lst.append(csp_vuln)
    ...
    for header_name in headers:
        ...
        for directive in directives:
            ...
            if (_contains_special_directive(directive_strip)
                ...
                continue
            # There must exists at least 2 parts otherwise we ignore the
            # value...
            if len(parts) < 2:
                continue
...
def merge_policies_dict(
        non_report_only_policies_dict,
        report_only_policies_dict):
    """
