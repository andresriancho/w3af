...
def show_chrome_pool_size(scan):
    scan.seek(0)
    ...
    for line in scan:
        match = CHROME_POOL_SIZE.search(line)
        if match:
            chrome_pool_free.append(int(match.group(1)))
            chrome_pool_used.append(int(match.group(2)))
            chrome_pool_timestamps.append(get_line_epoch(line))
    ...
    if not chrome_pool_timestamps:
        print('No chrome pool size data found')
        return

    print('Chrome pool size')
    print('')
...
def show_chrome_memory_usage(scan):
    ...
    for line in scan:
        ...
        if match:
            chrome_memory_usage.append(float(match.group(1)))
