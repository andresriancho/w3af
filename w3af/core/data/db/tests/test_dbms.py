...
class TestDBMS(unittest.TestCase):
    ...
    def test_update_update_rowcount(self):
        ...
        result = db.execute(
            'UPDATE TEST SET data = ? WHERE id = ?', ('b', 1)).result()
        ...
        #
        result1 = db.execute(
            'UPDATE TEST SET data = ? WHERE id = ?', ('c', 1)).result()
        result2 = db.execute(
            'UPDATE TEST SET data = ? WHERE id = ?', ('nope', 3)).result()
    ...
    def test_performance_with_multiple_cursors(self):
        ...
        for i in xrange(10000):
            result = db.execute(
                'UPDATE TEST SET data = ? WHERE id = ?', ('%s' %
                                                          i, 1)).result()
            self.assertEqual(result.rowcount, 1)
