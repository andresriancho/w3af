...
class CookieRequest(FuzzableRequest):
    """
    This class represents a fuzzable request that allows the user to easily
    modify HTTP request cookies.

    :author: Andres Riancho (andres.riancho@gmail.com)
    """
    @classmethod
    def from_parts(cls, url, method, post_data, cookies):
        # We want to modify HTTP cookies only in GET+QS requests
        if post_data:
            raise ValueError('Failed to create CookieRequest.')

        if method != 'GET':
            raise ValueError('Failed to create CookieRequest.')

        return cls(url, method=method, cookies=cookies, post_data=None)
    ...
    def get_dc(self):
        ...
        :return: A reference to the DataContainer object which will be used for
                 ...
                 order to allow fuzzing of cookies, cookies, post-data, etc.
        """
    ...
    def set_dc(self, data_container):
        ...
        """
        self.set_cookie(data_container)

    def __repr__(self):
        return ('<Cookies request | %s | %s>' %
