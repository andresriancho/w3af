...
class Base:
    ...
    types = {} # key = data class name, value = data class
    file_types = {} # key = data class, value = file class
    f_encode = {} # key = data class, value = function to_block
    f_decode = {} # key = data class, value = function from_block
    ...
    def _register_class(self,class_obj,class_file):
        ...
        class_file is the class used to manage the file thats hold
           the data of this class (eg IntegerFile in buzhug_classes)
        """
    ...
    def create(self,*fields,**kw):
        ...
        fields are couples (field_name,field_type)
        ...
        for (field_name,field_type) in self.fields.iteritems():
            self._validate_field(field_name,field_type)
        # add two internal fields, __id__ and __version__
        ...
        if not os.path.exists(self.name):
            os.mkdir(self.name)
        # save information in file __info__
        self._save_info()
        # create index file
    ...
    def _save_info(self):
        """Save information in the file __info___"""
        _info = open(self.info_name,'wb')
        fields = []
        for k in self.field_names:
            if isinstance(self.fields[k],Base):
                fields.append((k,'<base>'+str(self.fields[k].name)))
            else:
                fields.append((k,self.fields[k].__name__))
    ...
    def _open(self,info):
        ...
        for (k,v) in fields:
            if v.startswith('<base>'):
                ...
                _db = Base(v[6:]).open()
                self._register_base(_db)
                self.fields[k] = _db.types[v[6:]]
            else:
    ...
    def insert(self,*args,**kw):
        ...
        rec = dict([(f,None) for f in self.fields.keys()])
    ...
    def add_field(self,field_name,field_type,after=None,default=None):
        ...
        self._validate(field_name,default)
    ...
    def _validate_field(self,name,typ):
        if name == '__id__':
            raise ValueError,'Field name "__id__" is reserved'
