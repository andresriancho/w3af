...
class TestBufferOverflow(PluginTest):
    ...
    _run_config = {
        'target': target + '?buf=',
        'plugins': {
             'audit': (PluginConfig('buffOverflow',),),
             }
        }
    ...
    def test_found_bo(self):
        ...
        vulns = self.kb.getData('buffOverflow', 'buffOverflow')
        self.assertEquals(1, len(vulns))
        
        # Now some tests around specific details of the found vuln
        vuln = vulns[0]
        self.assertEquals('Buffer overflow vulnerability', vuln.getName())
        self.assertEquals("buf", vuln.getVar())
