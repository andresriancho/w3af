...
BROWSE_URLS = (
               ('GET', 'http://moth/w3af/discovery/spider_man/javascriptredirect.html', None),
               ('GET', 'http://moth/w3af/audit/sql_injection/select/sql_injection_integer.php', 'id=1'),
               ('POST', 'http://moth/w3af/discovery/spider_man/data_receptor_js.php', 'user=abc'),
              )
...
class TestSpiderman(PluginTest):
    ...
    _run_configs = {
        'cfg': {
            'target': base_url,
            'plugins': {'discovery': (PluginConfig('spider_man'),)}
            }
        }
