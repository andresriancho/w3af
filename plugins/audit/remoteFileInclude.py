...
class remoteFileInclude(baseAuditPlugin):
    ...
    def getOptionsXML(self):
        ...
        <OptionList>\
            <Option name="rfiUrl">\
                <default>'+self._rfiUrl+'</default>\
                <desc>Remote URL to include when testing for remote file inclusion.</desc>\
                <type>string</type>\
            </Option>\
            <Option name="rfiResult">\
                <default>'+self._rfiResult+'</default>\
                <desc>Expected result of inclusion</desc>\
                <type>string</type>\
                <help>For example, at RfiUrl the user entered :\
http://foo/toBeIncluded.txt\
...
mfsa09a059mu2\
            ...
            <Option name="listenAddress">\
                <default>'+self._listenAddress+'</default>\
                <desc>IP address that the webserver will use to receive requests</desc>\
                <type>string</type>\
                <help>When the two previous settings are left unconfigured, w3af runs a webserver\
                to serve the files to the target web app when doing remote file inclusions. This \
                setting configures where the webserver is going to listen.</help>\
            </Option>\
            ...
            <Option name="usew3afSite">\
                <default>'+str(self._usew3afSite)+'</default>\
                <desc>Use w3af site to test remote file inclusion. If set to True this will override all other options.</desc>\
                <type>boolean</type>\
                <help></help>\
            </Option>\
        </OptionList>\
