    ...
    def getOptionsXML(self):
        ...
        <OptionList>\
            <Option name="stopOnFirst">\
                ...
                <desc>Stop on the first successfull file upload</desc>\
                <type>boolean</type>\
                <help>The default value is usually a good idea, because if we can upload a file to a directory, the chances are that we can upload to every directory; and if this is the case, we would get a lot of vulnerabilities reported, that are really only one.</help>\
            </Option>\
        </OptionList>\
        '
